...
import { addDiscussionComment } from "@/app/tartismalar/actions";
  ...
  return (
    <section className="mt-10">
      <h2 className="flex items-center gap-2 font-serif text-2xl font-semibold text-paper">
        <MessageCircle size={20} className="text-amber" /> Yorumlar
      ...
      <div className="mt-8 space-y-6">
        {roots.length ? (
          roots.map((c) => <DiscussionCommentNode key={c.id} comment={c} depth={0} children={byParent} discussionId={discussionId} slug={slug} signedIn={signedIn} />)
        ) : (
    ...
    <article className={depth ? "ml-3 border-l border-white/10 pl-4 sm:ml-6 sm:pl-6" : ""}>
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {signedIn && (
            <button onClick={() => setReply(!reply)} className="mt-2 inline-flex items-center gap-1 rounded-full bg-white/5 px-3 py-1 text-xs font-medium text-zinc-400 hover:bg-white/10 hover:text-amber">
              <CornerDownRight size={12} /> {reply ? "Kapat" : "Yanıtla"}
            </button>
          )}
          {reply && <DiscussionCommentForm discussionId={discussionId} slug={slug} parentId={comment.id} compact />}
        </div>
      </div>
      {replies.length > 0 && (
        <div className="mt-5 space-y-5">
          {replies.map((child) => (
            <DiscussionCommentNode key={child.id} comment={child} depth={depth + 1} children={children} discussionId={discussionId} slug={slug} signedIn={signedIn} />
          ))}
        </div>
      )}
    </article>
