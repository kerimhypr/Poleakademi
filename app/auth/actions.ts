...
const credentials = z.object({
  ...
  displayName: z.string().trim().min(2, "Görünen ad en az 2 karakter olmalı.").max(60).optional(),
});
...
export async function requestPasswordReset(_: AuthState, formData: FormData): Promise<AuthState> {
  ...
  const { error } = await supabase.auth.resetPasswordForEmail(email.data, {
    redirectTo: `${await getOrigin()}/auth/confirm?next=/sifre-yenile`,
  });
  if (error) return { error: "İşlem tamamlanamadı. Lütfen tekrar dene." };
  return { message: "Hesap varsa, şifre yenileme bağlantısı e-postana gönderildi." };
}
...
export async function updatePassword(_: AuthState, formData: FormData): Promise<AuthState> {
  ...
  if (error) return { error: "Şifre güncellenemedi. Bağlantı süresi dolmuş olabilir." };
  redirect("/profil");
}
